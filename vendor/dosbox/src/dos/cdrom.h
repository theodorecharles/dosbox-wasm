...
/* Emscripten's SDL compatibility layer intentionally omits the obsolete
 * physical-CD API. ISO support still uses these frame conversion helpers. */
#define MSF_TO_FRAMES(M, S, F) ((M) * 60 * 75 + (S) * 75 + (F))
#define FRAMES_TO_MSF(frames, M, S, F) do { \
...
static inline const char* SDL_CDName(int /*drive*/) { return ""; }
#endif
