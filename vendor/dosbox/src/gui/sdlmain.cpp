...
#include "cross.h"
#include "SDL.h"
#if defined(__EMSCRIPTEN__) && !defined(SDL_INIT_CDROM)
...
extern "C" int SDL_LockYUVOverlay(SDL_Overlay*) { return -1; }
extern "C" void SDL_UnlockYUVOverlay(SDL_Overlay*) {}
extern "C" int SDL_DisplayYUVOverlay(SDL_Overlay*, SDL_Rect*) { return -1; }
extern "C" void SDL_FreeYUVOverlay(SDL_Overlay*) {}
extern "C" int SDL_WaitEvent(SDL_Event* event) { return SDL_PollEvent(event); }
#endif
...
void GFX_Events() {
	...
	while (SDL_PollEvent(&event)) {
#if defined(__EMSCRIPTEN__)
		/* Give keyboard-driven DOS games the same movement layout as the rest
		 * of the browser portfolio while retaining the arrow keys themselves. */
		if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) {
			switch (event.key.keysym.sym) {
			case SDLK_w: event.key.keysym.sym = SDLK_UP; break;
			case SDLK_s: event.key.keysym.sym = SDLK_DOWN; break;
			case SDLK_a: event.key.keysym.sym = SDLK_LEFT; break;
			case SDLK_d: event.key.keysym.sym = SDLK_RIGHT; break;
			default: break;
			}
		}
#endif
